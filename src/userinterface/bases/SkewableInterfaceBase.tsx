...
export function SkewableInterfaceBase<T extends ISkewable>(props: ISkewableInterfaceBaseProps<T>) {
    function setX(newValue: number) {
        const scaledValue = newValue / 10;

        if (props.value.combineSkew) {
            props.onChange({ ...props.value!, skewX: scaledValue, skewY: scaledValue });
        } else {
        ...
        <Box
            ...
            <Box gridArea="Combine">
                <FormControlLabel
                    ...
                    label="Combine"
                    checked={props.value.combineSkew}
